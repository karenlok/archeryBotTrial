...
class MainDialog extends ComponentDialog {
    ...
    async introStep(stepContext) {
        ...
        const messageText = stepContext.options.restartMsg ? stepContext.options.restartMsg : 'What can I help you with today?\nSay something like "Are there any archery competitions on 1 January 2023 in Hong Kong?"';
    ...
    async actStep(stepContext) {
        ...
        case 'searchCompetition': {
            ...
            const locationEntities = this.cluRecognizer.getLocation(cluResult);
            const dateEntities = this.cluRecognizer.getDate(cluResult);
            // const nameEntities = this.cluRecognizer.getName(cluResult);

            // Show a warning for Origin and Destination if we can't resolve them.
            // await this.showWarningForUnsupportedCities(stepContext.context, dateEntities, locationEntities);

            // Initialize BookingDetails with any entities we may have found in the response.
            competitionDetails.date = dateEntities;
            competitionDetails.location = locationEntities;
            console.log('hihi');
            // bookingDetails.travelDate = this.cluRecognizer.getTravelDate(cluResult);
            console.log('CLU extracted these booking details:', JSON.stringify(competitionDetails));

            // Run the BookingDialog passing in whatever details we have from the CLU call, it will fill out the remainder.
            return await stepContext.beginDialog('bookingDialog', competitionDetails);
        }

        case 'applyMembership': {
            // We haven't implemented the GetWeatherDialog so we just display a TODO message.
            const getWeatherMessageText = 'TODO: apply for membership here';
    ...
    async finalStep(stepContext) {
        ...
        if (stepContext.result) {
            ...
            const msg = 'Thank you for using this service.'
